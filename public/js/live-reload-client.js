...
(() => {
  ...
  socket.on("reload page", async () => {
    ...
    Alpine.store('reloadState').capture();
    htmx.ajax("GET", location.pathname, {
      ...
      Alpine.store('reloadState').restore();
    });
  });
  ...
  socket.on("reload script", async (script) => {
    ...
    if (el) {
      ...
      const newScriptContent = await fetchWithRetry(script).then((r) =>
        r.text(),
      );
      ...
      const newSriptContent = await fetchWithRetry(script).then((r) =>
        r.text(),
      );
      el.innerHTML = newSriptContent;
    }
  });
